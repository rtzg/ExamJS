...
var examJS = {
    ...
    antiCheat: (openPage = '') => {
            ...
            return function(n) {
                ...
                function K() {
                    if (L(), F.url) window.location.href = F.url;
                        ...
                        try { window.opener = null, window.open("", "_self"), window.close(), window.history.back() } catch (n) { console.log(n) }
        ...
        if (Notification.permission === "denied") {
            alert(tellUserToEnableNotifications);
            location.reload();
        }
        // if user allow to show notifications, show them
        if (Notification.permission === "granted") {
            var notification = new Notification(examJS.notifications.test[0], {
                body: examJS.notifications.test[1],
            });
            alert(examJS.notifications.test[2]);
            // wait for user to click on notification
    ...
    onlyFoucs: () => {
        ...
        document.body.onblur = () => {
            ...
            // if user allow to show notifications, show them
            if (Notification.permission === "granted") {
                var notification = new Notification(examJS.notifications.focus.title, {
                    body: examJS.notifications.focus.notFoucsing
                });
                timeouted = setTimeout(() => {
                    notification.close();
                    var notification = new Notification(examJS.notifications.focus.title, {
                        body: examJS.notifications.focus.submitted
                    });
                }, 20000);
            }
        };

        // if user is focused on the page
        document.body.onfocus = () => {
            console.log("focus");
            var notification = new Notification(examJS.notifications.focus.title, {
                body: examJS.notifications.focus.foucsing
            });
    ...
    notifications: {
        ...
        focus: {
            ...
            foucsing: "Seems like you are focused on the page. The timer will be reset.\nYou don't need to click on the notification.",
            submitted: "Your answer has been submitted."
        }
    }
};

var tellUserToEnableNotifications = "Anti-cheat is running in background.\nPlease click on the notification to continue.";
...
const createExamWidget = (element, json = {}) => {
    ...
    if (!partition) {
        ...
        for (let i = 0; i < questions.length; i++) {
            examJS.correctAnswers.push(questions[i].answer);
            id = i;
            var {
                type,
                text,
            ...
            if (questionType === 'FillQ') {
                $(element).append(FillQuestionElement.replace(/\[id\]/g, id).replace(/\[question\]/g, text).replace(/\[type\]/g, 'text').replace(/\[prompt\]/g, prompt).replace(/\[required\]/g, answer != undefined ? '<span class="required">*</span>' : ''));
            }
            if (questionType === 'MCQ') {
                $(element).append(MCQuestionElement.replace(/\[id\]/g, id).replace(/\[question\]/g, text).replace(/\[type\]/g, 'radio').replace(/\[prompt\]/g, prompt).replace(/\[required\]/g, answer != undefined ? '<span class="required">*</span>' : ''));
                for (let j = 0; j < option.length; j++) {
                    $(`#question${id}`).append(`<label><input type="radio" name="question${id}c" value="${option[j]}">${option[j]}</label>`);
                }
            }
        }
        $(element).append(submitElement);
    } else {
        $(element).append(`<h1>${title}</h1>`);
        examJS.quantity = questions.length;
        ...
        // create the parts with divs and h2
        for (let i = 0; i < parts.length; i++) {
            $(element).append(`<div id="part${i}"><h2>${parts[i]}</h2></div>`);
        }
        ...
        for (let i = 0; i < questions.length; i++) {
            ...
            if (questionType === 'FillQ') {
                $(`#part${parts.indexOf(part)}`).append(FillQuestionElement.replace(/\[id\]/g, id).replace(/\[question\]/g, text).replace(/\[type\]/g, 'text').replace(/\[prompt\]/g, prompt).replace(/\[required\]/g, answer != undefined ? '<span class="required">*</span>' : ''));
                examJS.correctAnswers.push(answer);
            }
            if (questionType === 'MCQ') {
                $(`#part${parts.indexOf(part)}`).append(MCQuestionElement.replace(/\[id\]/g, id).replace(/\[question\]/g, text).replace(/\[type\]/g, 'radio').replace(/\[prompt\]/g, prompt).replace(/\[required\]/g, answer != undefined ? '<span class="required">*</span>' : ''));
                for (let j = 0; j < option.length; j++) {
                    $(`#question${id}`).append(`<label><input type="radio" name="question${id}" value="${option[j]}">${option[j]}</label>`);
                }
                examJS.correctAnswers.push(answer);
            }
        }
...
var checkAnswer = (idFormat = 'question') => {
    ...
    json.correctPercentage = Math.round(json.correct / json.quantity * 100) + '%';
    json.wrongPercentage = Math.round(json.wrong / json.quantity * 100) + '%';
    json.notAnswredPercentage = Math.round(json.notAnswred / json.quantity * 100) + '%';
    json.answered = answered;
    json.noCorrectAnswerQuestions = undefinedAnswers;
    json.ncaq = undefinedAnswers.length;
    // get each part and its answers , correct , wrong and not answered
    var parts = [];
    for (let i = 0; i < answers.length; i++) {
        if (!parts.includes(answers[i][1])) {
            parts.push(answers[i][1]);
        }
    }
    json.parts = [];
    for (let i = 0; i < parts.length; i++) {
        ...
        for (let j = 0; j < answers.length; j++) {
            if (answers[j][1] === parts[i]) {
                if (answers[j][0] === true) {
                    part.correct++;
                } else {
                    part.wrong++;
                }
                if (answers[j][2] === '' || answers[j][2] === 'Null' || answers[j][2] === undefined || answers[j][2] === null) {
                    part.notAnswred++;
                }
                part.answers.push(answers[j]);
            }
        }
